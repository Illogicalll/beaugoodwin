<script lang="ts">
  ...
  onMount(() => {
    if (typeof window !== 'undefined') {
      isMobile = Bowser.getParser(window.navigator.userAgent).getResult().platform.type === 'mobile' ? true : false;
...
<div class="toggle-bar" style="position: fixed; bottom: {isBottomQuarter ? '3%' : '-400px'}; background-color: {backgroundColor}; border-radius: 9px; padding: 5px; opacity: {isBottomQuarter ? '1' : '0.4'};  transition: bottom 0.4s ease, opacity 0.1s ease, background-color 0.3s ease;">
  <ToggleGroup.Root size="lg" type="single" onValueChange={handlePageChange}>
    <Tooltip.Root>
      <Tooltip.Trigger>
        <ToggleGroup.Item value="" aria-label="Home" data-state={curPage === "/" ? 'on' : 'off'} disabled={curPage === "/"} >
    ...
    <Tooltip.Root>
      <Tooltip.Trigger>
        <ToggleGroup.Item value="blog" aria-label="Blog" data-state={curPage !== "/" ? 'on' : 'off'} disabled={curPage === "/blog/"}>
